...
obj/Effects/KenShockwave
	...
	var/Size=1
	New()
		var/Fade_Time=16//How long it takes for the Shockwave to fade away/delete
		animate(src,transform=matrix()*0.1)//The Shockwave starts out small
		spawn(1)
			animate(src,alpha=0,time=Fade_Time,transform=matrix()*Size)//Enlarges overtime and then fades away
			spawn(Fade_Time)
				del src
proc
	KenShockwave(atom/M,icon='KenShockwave.dmi',Size=1,PixelX=0,PixelY=0)  //M is the person that makes t
		if(Size>5)
			Size=5
...
obj/Effects/KenShockwave2
	...
	New()
		...
		spawn(1)
			animate(src,alpha=0,time=Fade_Time,transform=matrix()*0.1)
			spawn(Fade_Time)
				del src
proc
	KenShockwave2(atom/M,icon='KenShockwave.dmi',Size=1,PixelX=0,PixelY=0)  //M is the person that makes t
		if(Size>5)
			Size=5
